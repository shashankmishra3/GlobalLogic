...
alert(JSON.stringfy(meetup, ['title', 'participants', 'name', 'place', 'number']);
```
...
alert(JSON.stringfy(meetup, function replacer(key, value){
    alert(`${key}: ${value}`);
...
**NOTE** -  the `replacer` function gets every key/value pair including nested objects and array items. It is applied recursively. The value of `this` inside `replacer` is the object that contains the current property.
...
now we need to *deserialize* it, to turn back into JavaScript object
